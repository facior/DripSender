...
from .. import branding, importer, updater, validate
...
class UpdateDialog(ModalDialog):
    """Informacja o nowej wersji, pobranie i instalacja."""

    def __init__(self, parent, store: Store, wydanie) -> None:
        self.store = store
        self.wydanie = wydanie
        self.plik = None
        super().__init__(parent, "Dostępna aktualizacja", 620, 520)
        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(0, weight=1)

        card = Card(
            self,
            "Nowa wersja " + wydanie.version,
            "Masz wersję " + branding.VERSION + ". Aktualizacja pobierze nowy plik "
            "programu i podmieni go po zamknięciu.",
        )
        card.grid(row=0, column=0, sticky="nsew", padx=16, pady=(16, 8))
        card.body.grid_rowconfigure(1, weight=1)

        ctk.CTkLabel(
            card.body, text="Co nowego", font=theme.font(11), text_color=theme.MUTED, anchor="w"
        ).grid(row=0, column=0, sticky="ew", pady=(0, 4))
        opis = textbox(card.body, height=150)
        opis.grid(row=1, column=0, sticky="nsew")
        opis.insert("1.0", wydanie.opis)
        opis.configure(state="disabled")

        self.progress = ctk.CTkProgressBar(
            card.body, height=8, corner_radius=4,
            progress_color=theme.ACCENT, fg_color=theme.SURFACE_HI,
        )
        ...
        self.status = ctk.CTkLabel(
            card.body, text="", font=theme.font(11), text_color=theme.MUTED,
        ...
        self.install_button = button(actions, "Pobierz i zainstaluj", self._pobierz, width=180)
        self.install_button.grid(row=0, column=0, padx=(0, 8))
        button(actions, "Pomiń tę wersję", self._pomin, kind="ghost", width=150).grid(
            row=0, column=2, padx=(0, 8)
        )
        button(actions, "Później", self._cancel, kind="ghost", width=110).grid(row=0, column=3)

    def _pomin(self) -> None:
        self.store.set_setting("update_skipped", self.wydanie.version)
        self.result = "pominieto"
        self.destroy()

    def _pobierz(self) -> None:
        self.install_button.configure(state="disabled", text="Pobieram...")
        self.progress.grid()
        self.status.configure(text="Łączę się z serwerem...", text_color=theme.ACCENT)

        # Wątek NIE dotyka Tkintera - zapisuje stan do zwykłych pól, a wątek główny
        # je odpytuje. Wołanie after() przy każdym kawałku pliku zakleszczało okno.
        self._stan_postepu: tuple[int, int] | None = None
        self._wynik = None

        def postep(pobrane: int, calosc: int) -> None:
            self._stan_postepu = (pobrane, calosc)

        def worker() -> None:
            try:
                plik = updater.pobierz(self.wydanie, postep=postep)
            except updater.UpdateError as exc:
                self._wynik = ("blad", str(exc))
            except Exception as exc:  # noqa: BLE001
                self._wynik = ("blad", "Nieoczekiwany błąd pobierania: " + str(exc))
            else:
                self._wynik = ("ok", plik)

        threading.Thread(target=worker, name="update", daemon=True).start()
        self.after(120, self._sledz_pobieranie)

    def _sledz_pobieranie(self) -> None:
        """Odpytuje stan pobierania z wątku głównego - jedyne bezpieczne podejście."""
        if not self.winfo_exists():
            return
        if self._stan_postepu:
            self._postep(*self._stan_postepu)
        wynik = self._wynik
        if wynik is None:
            self.after(120, self._sledz_pobieranie)
            return
    ...
    def _postep(self, pobrane: int, calosc: int) -> None:
        self.progress.set(pobrane / calosc)
        self.status.configure(
            text="Pobrano "
            + str(round(pobrane / 1024 / 1024, 1))
            + " z "
            + str(round(calosc / 1024 / 1024, 1))
            + " MB",
            text_color=theme.ACCENT,
        )

    def _blad(self, komunikat: str) -> None:
        self.progress.grid_remove()
        self.install_button.configure(state="normal", text="Spróbuj ponownie")
        self.status.configure(text=komunikat, text_color=theme.ERR)
        self.store.log("error", "Aktualizacja: " + komunikat)

    def _gotowe(self, plik) -> None:
        self.plik = plik
        self.progress.set(1)
        self.status.configure(
            text="Pobrano i sprawdzono sumę kontrolną. Program zamknie się i uruchomi "
            "ponownie w nowej wersji.",
            text_color=theme.OK,
        )
        self.install_button.configure(state="normal", text="Zainstaluj i uruchom", command=self._instaluj)

    def _instaluj(self) -> None:
        if not messagebox.askyesno(
            "Zainstalować teraz?",
            "Program zamknie się, podmieni swój plik i uruchomi ponownie.\n\n"
            "Kampania w toku zostanie przerwana - po starcie wznowi się sama, "
            "jeśli masz włączone wznawianie.",
            parent=self,
        ):
            return
        try:
            updater.zainstaluj(self.plik)
        except updater.UpdateError as exc:
            self._blad(str(exc))
            return
        self.store.log("info", "Instaluję aktualizację do wersji " + self.wydanie.version + ".")
        self.result = "instaluje"
        self.destroy()
